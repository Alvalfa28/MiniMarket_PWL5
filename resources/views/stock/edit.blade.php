...
<body class="bg-gray-100 dark:bg-gray-900">
    ...
    <div class="flex justify-center items-center min-h-screen">
        <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6 w-1/2">
            ...
            <form action="{{ route('stock.update', ['branchId' => $branchId, 'stockId' => $stock->id]) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mt-4">
                    ...
                    <select name="id_produk" id="id_produk" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:bg-gray-800 dark:text-gray-200">
                        @foreach ($produkList as $produk)
                            <option value="{{ $produk->id }}" {{ $produk->id == $stock->produk->id ? 'selected' : '' }}>
                                {{ $produk->nama_produk }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mt-4">
                    ...
                    <input type="number" name="harga_produk" id="harga_produk" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:bg-gray-800 dark:text-gray-200" value="{{ $stock->produk->harga_produk }}" required>
                </div>
